...
import { FileDiffRow } from "../../git/components/DiffViewer";
import { AuthorAvatar } from "../../git/components/AuthorAvatar";

interface SshCommitDetailPanelProps {
  connectionId: string;
  onClose: () => void;
  maximized: boolean;
...
export function SshCommitDetailPanel({
  connectionId,
  onClose,
  maximized,
  onToggleMaximize,
}: SshCommitDetailPanelProps) {
  const selectedHash = useSshGitStore((s) => s.selectedCommitHash);
  ...
  useEffect(() => {
    if (selectedHash && connectionId) {
      fetchCommitDetail(connectionId, selectedHash);
    }
  }, [selectedHash, connectionId, fetchCommitDetail]);
  ...
  const graphData = useSshGitStore((s) => s.graphData);
  ...
  const handleCopy = useCallback(async (text: string, field: string) => {
    ...
    try {
      ...
      console.warn("[SshCommitDetailPanel] Clipboard write failed:", e);
